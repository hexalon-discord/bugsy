...
var ProcessManager = class {
  constructor(message2, content, bugsy, options = {}) {
  ...
  filterSecret(string) {
    string = string.replace(
      new RegExp(this.bugsy.client.token, "gi"),
      "[accesstoken was hidden]"
    );
    if (this.bugsy.options.secrets) {
      for (const el of this.bugsy.options.secrets) {
...
async function main(message2, parent2) {
  ...
  let summary = `Bugsy v${version}, discord.js \`${import_discord4.version}\`, \`Node.js ${process.version}\` on \`${process.platform}\`
...
async function err(message, parent) {
  ...
  fs.readFile(`src/data/logs/err_${errorId}.json`, 'utf8', async (err, data) => {
    ...
    .addFields({name: `Error Information`,value:`<:js:1233487053921190100>**Error Details** ${ero}\n<:timer:1233486986166403153>**Time** <t:${Math.floor(time / 1000)}:R>`},)
    .setColor("#2B2D31")
    if (ero.length > 1024) {
  ...
  constructor(client2, options) {
    ...
    client2.once("ready", (client3) => {
      if (!this.owners.length) {
        console.warn("[ Bugsy ] Owners not given. Fetching from Application.");
        client3.application.fetch().then((data) => {
          if (!data.owner) {
            return console.warn("[ Bugsy ] Falied to owner data.");
          }
          ...
          console.info(
            `[ Bugsy ] Fetched ${this.owners.length} owner(s): ${this.owners.length > 3 ? this.owners.slice(0, 3).join(", ") + ` and ${this.owners.length - 3} more owners` : this.owners.join(", ")}`
          );
        });
      }
    });
